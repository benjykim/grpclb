...
class Client(object):
    ...
    def init_with_net_profile(self, profile_path='network.json'):
        ...
        # Init channels jyk
        channels = self.get_net_info('channels')
        for name in channels:
            _logger.debug("create channel with name={}".format(name))
            channel = Channel(name=name, client=Client)
    ...
    async def chaincode_invoke(self, requestor, channel_name, peers, args,
                               cc_name, avg_latency, cc_type=CC_TYPE_GOLANG,
        ...
        # jyk
        print(str((time.time() - start)*1000 + avg_latency) + ',')
        del responses[1]
        ...
        # if proposal was not good, return
        if not all([x.response.status == 200 for x in res]):
            print("if proposal was not good")
            return res[0].response.message
